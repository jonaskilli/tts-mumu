...
var DualKeyManager = (function() {
  // 兜底默认配置，和原代码完全对齐
  var defaultConfig = {
      endpoint: "https://cnb.cool/ai/chat/completions",
      model: "hy3-preview",
      key: "cnb"
  };
...
//      pools.aliasAnalyze.list = fillApiListToMinCount(pools.aliasAnalyze.list, MIN_CONCURRENT_COUNT);
  ...
  // 新增：【核心】获取当前场景的可用API密钥列表（CNB 模式：统一返回混元3配置，不再读取 miyue.txt 密钥池）
  function getAvailableApiList(scene, needCount) {
      // 预热 csrftoken（带缓存，避免并发线程重复拉取）
      getCnbCsrfToken();
      var count = needCount && needCount > 0 ? needCount : (parseInt(bingfa, 10) || MIN_CONCURRENT_COUNT);
      if (count < 1) count = MIN_CONCURRENT_COUNT;
      var result = [];
      for (var i = 0; i < count; i++) {
          result.push({ endpoint: CNB_CONFIG.baseUrl + CNB_CONFIG.apiPath, model: CNB_CONFIG.model, key: "cnb" });
      }
      return result;
  }
...
// ===================== CNB 接口配置（替代原智谱AI配置，直接调用混元3） =====================
// 对应 python 包 cnb.py / cookie_keeper.py：
//   1) 访问 explore 页面，从 __NEXT_DATA__ 提取 csrftoken
...
var CNB_CONFIG = {
    ...
    csrfkey: "",  // 取自 cookie.json，可被 cnb_cookie.json 覆盖
    model: "hy3-preview"                            // 可选：hy3-preview / glm-5.2 / deepseek-v4-pro / deepseek-v4-flash
};
// 尝试从外部 cookie 文件加载 csrfkey（支持 {"cookie":"csrfkey=xxx"} 或 {"csrfkey":"xxx"}）
try {
    var _cnbCookieRaw = ttsrv.readTxtFile("cnb_cookie.json");
...
// csrftoken 持久化文件（与阅读脚本通用：cnb_cookie.json 存 csrfkey，cnb_token.json 存 csrftoken）
var CNB_TOKEN_FILE = "cnb_token.json";
// 恢复上次持久化的 csrftoken（脚本重载后复用，避免每次都请求 explore）
var _cnbCsrfToken = "";
try {
    var _cnbTokRaw = ttsrv.readTxtFile(CNB_TOKEN_FILE);
...
function getCnbCsrfToken() {
    // 只要 csrftoken 与 csrfkey 都在，就一直复用，避免每次请求 explore
    if (_cnbCsrfToken && CNB_CONFIG.csrfkey) return _cnbCsrfToken;
    try {
        var headers = { "User-Agent": CNB_UA, "Accept": "text/html,*/*", "Cookie": "csrfkey=" + CNB_CONFIG.csrfkey };
        var resp = ttsrv.httpGet(CNB_CONFIG.exploreUrl, headers);
        // ★ 修复：csrfkey 在 Set-Cookie 响应头里（对齐"响应头侧存与解析方案"），
        //   页面 __NEXT_DATA__ 并不回显 csrfkey，原逻辑从 HTML 取永远为空。
        //   okhttp 的 Headers.get() 大小写不敏感，直接取 set-cookie 即可。
        var scRaw = resp.headers().get("set-cookie") || "";
        var km = scRaw.match(/csrfkey=([a-f0-9]{32})/i);
        if (km && km[1] && km[1] !== CNB_CONFIG.csrfkey) {
            CNB_CONFIG.csrfkey = km[1];
            try { ttsrv.writeTxtFile("cnb_cookie.json", JSON.stringify({ cookie: "csrfkey=" + km[1] })); } catch (e) {}
            console.log("【CNB】已从响应头 set-cookie 更新 csrfkey");
        }
        // csrftoken 仍从页面 __NEXT_DATA__ 提取
        var html = resp && resp.body ? String(resp.body().string() || "") : "";
        if (html) {
            var m = html.match(/<script[^>]*id=["']__NEXT_DATA__["'][^>]*>([\s\S]*?)<\/script>/);
            if (m) {
                var data = JSON.parse(m[1]);
                var pageProps = data && data.props && data.props.pageProps;
                var token = pageProps && pageProps.csrftoken;
                if (token) {
                    _cnbCsrfToken = token;
                    // 持久化 csrftoken 到 cnb_token.json，供脚本重载后复用
                    try { ttsrv.writeTxtFile(CNB_TOKEN_FILE, JSON.stringify({ csrftoken: token })); } catch (e) {}
                    console.log("【CNB】已获取 csrftoken");
                    return token;
                }
            }
        }
    } catch (e) { console.log("【CNB】获取 csrftoken 失败：" + e.message); }
    return _cnbCsrfToken;
}

// 构建 CNB 请求头（每次取最新 csrftoken）
function buildCnbHeaders() {
...
"\n\n";
  ...
  function buildNameAnalyzeRequest(apiConfig) {
    ...
    return {
      endpoint: CNB_CONFIG.baseUrl + CNB_CONFIG.apiPath,
      data: requestData,
      headers: buildCnbHeaders()
    };
  }
  function parseNameAnalyzeResponse(response) {
    // CNB 鉴权失败（401/403）时清空 token 与持久化文件，触发下一轮刷新重试
    if (typeof response.code === "function") {
      var _code = response.code();
      if (_code === 401 || _code === 403) {
        _cnbCsrfToken = "";
        try { ttsrv.writeTxtFile(CNB_TOKEN_FILE, ""); } catch (e) {}
        throw new Error("CNB鉴权失败(" + _code + ")，将刷新凭证重试");
      }
    }
...
CharacterManager.prototype.processCharacter = function (fullText, characterId, allDialogues, chapterFullContent) {
  ...
  if (bieming !== 0 && newCharacterName !== "未知" && Object.keys(pendingAliasNames).length > 0 && pendingAliasContext) {
    ...
    function buildBatchRequest(apiConfig) {
      ...
      return { endpoint: CNB_CONFIG.baseUrl + CNB_CONFIG.apiPath, data: requestData, headers: buildCnbHeaders() };
    }
