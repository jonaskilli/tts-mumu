 ...
 *
 * 定 32dp（目目 09-17 二次反馈「还是窄」，16→24→32）：底部面板满宽贴屏，
 * 边距紧挨着原始屏幕边，观感上比同样的数值在居中卡片里显得窄——要拉到 32dp
 * 才有居中卡片「两层边距」的宽松感。360dp 屏上内容区 296dp，仍宽于回退
 * 居中卡片能拿到的 ~264dp（24+24 两层叠加），故不回退居中形态。
 */
 ...
 * 都经由本组件，改这一处即全部生效。
 *
 * 左右边距口径：**面板内容统一 32dp**（标题行 / 内容区 / 按钮行共用 `PANEL_HORIZONTAL_PADDING`；
 * ✕ 的 48dp 触摸区自带 12dp 内缩、标题行给 end=20，图标正好落在 32dp 右缘线上）。
 * 16→24→32 三轮加宽（目目 09-17 连续两次「还是窄」）——满宽贴屏的面板要 32dp
 * 才有居中卡片两层边距的宽松感。内层组件（搜索框、列表条目）在底部形态下
 * **不要再自加横向内边距**，由本外壳一处说了算。
 *
        ...
        Box(
            ...
            Surface(
                ...
                Column(
                    ...
                    // end=20 是给 ✕ 的 48dp 触摸区留的：IconButton 自带 12dp 内缩，
                    // 20 + 12 = 32 ⇒ ✕ 图标正好落在与内容区同一条右缘线上
                    Row(
                        Modifier
                            .fillMaxWidth()
                            .padding(
                    ...
                    // 否则叠出来还是多套左缘线。首版 16dp → 24dp → 32dp（目目两轮「还是窄」）
                    Box(
                        Modifier
