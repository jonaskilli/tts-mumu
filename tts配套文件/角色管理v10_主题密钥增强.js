...
var EditorJS = {
    ...
    'onLoadUI': function (ctx, linearLayout) {
        ...
        // 用 AlertDialog 显示完整错误信息（Toast 会截断长文本）
        function showErrorDialog(title, message) {
            ...
            tv.setPadding(dipToPx(24), dipToPx(16), dipToPx(24), dipToPx(8));
   ...
   currentTestResultPopup.showAtLocation(anchor, android.view.Gravity.TOP | android.view.Gravity.CENTER_HORIZONTAL, 0, yOffset);
            ...
            // 小按钮工厂
            function createSmallButton(text, color, borderless) {
            ...
            var titleText = createDialogTitle("密钥管理");
            ...
            var importBtn = createSmallButton("📥 导入", "#757575", true);
            importBtn.setOnClickListener(new android.view.View.OnClickListener({
                onClick: function(v) {
                    importKeysDialog();
                }
            }));
            titleRow.addView(importBtn);

            var exportBtn = createSmallButton("📤 导出", "#757575", true);
            exportBtn.setOnClickListener(new android.view.View.OnClickListener({
                onClick: function(v) {
                    exportKeysDialog();
                }
            }));
            titleRow.addView(exportBtn);
            ...
            for (var gi2 = 0; gi2 < keyGroups.length; gi2++) {
                (function(grp) {
                    ...
                    if (isCurrent) {
                        rowBg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.tint));
                        rowBg.setStroke(dipToPx(1.5), android.graphics.Color.parseColor(RMTHEME.cur.main));
                    } else {
                    ...
                    // 第一行：圆点/对勾 + 名称 + 测试 + 编辑 + 删除
                    var topRow = new android.widget.LinearLayout(ctx);
                    topRow.setOrientation(android.widget.LinearLayout.HORIZONTAL);
                    topRow.setGravity(android.view.Gravity.CENTER_VERTICAL);
                    topRow.setLayoutParams(new android.widget.LinearLayout.LayoutParams(
                        android.widget.LinearLayout.LayoutParams.MATCH_PARENT,
                        android.widget.LinearLayout.LayoutParams.WRAP_CONTENT
                    ));
                    ...
                    if (isCurrent) {
                        ...
                        checkText.setTextColor(android.graphics.Color.parseColor(RMTHEME.cur.main));
                        checkText.setGravity(android.view.Gravity.CENTER);
                        checkText.setPadding(0, 0, dipToPx(8), 0);
                        topRow.addView(checkText);
                    } else {
                            ...
                            new java.lang.Thread(new java.lang.Runnable({
                                run: function() {
                                    ...
                                    runOnUiThreadSafe(function() {
                                        ...
                                        Toast.makeText(ctx, (result.ok ? "测试成功：" : "测试失败：") + (result.message || "未知结果"), Toast.LENGTH_LONG).show();
                                    });
                                }
                            })).start();
                        }
                    ...
                    viewEditBtn.setOnClickListener(new android.view.View.OnClickListener({
                        onClick: function(v) {
                            try {
                                ...
                                var copyBtn = createTextBtn("复制", RMTHEME.cur.main);
                                ...
                                var saveBtn = createTextBtn("保存", RMTHEME.cur.main);
                                applyActionBtnLayout(saveBtn, 2);
                    ...
                    if (parts.length >= 3) {
                        ...
                        label = host + "  *尾" + tail.substring(Math.max(0, tail.length - 4));
                    } else {
                        label = "智谱内置";
                    }
                    if (label && !grp.ifc) {
                        ...
                        srcText.setTextColor(android.graphics.Color.parseColor("#9e9e9e"));
                        srcText.setSingleLine(false);
                        srcText.setPadding(dipToPx(15), dipToPx(2), 0, 0);
                        srcText.setLayoutParams(new android.widget.LinearLayout.LayoutParams(
                            android.widget.LinearLayout.LayoutParams.MATCH_PARENT,
                            android.widget.LinearLayout.LayoutParams.WRAP_CONTENT
                        ));
                        row.addView(srcText);
            ...
            keyManageDialog = dialog;
            applyDialogRoundCorner(dialog);
        }

        // 导出密钥：写入本地文件 密钥导出_YYMMDD.json
        function exportKeysDialog() {
            var kl = getKeyMapAndList();
            if (kl.list.length === 0) {
                Toast.makeText(ctx, "暂无密钥可导出", Toast.LENGTH_SHORT).show();
                return;
            }
            var sdf = new java.text.SimpleDateFormat("yyMMdd");
            var dateStr = sdf.format(new java.util.Date());
            var EXPORT_FILE = "密钥导出_" + dateStr + ".json";
            var exportArr = [];
            for (var i = 0; i < kl.list.length; i++) {
                var name = kl.list[i];
                var item = kl.map[name];
                exportArr.push([name, { keyCode: item.keyCode || "", value: item.value || "" }]);
            }
            var jsonStr = JSON.stringify(exportArr, null, 2);
            try {
                ttsrv.writeTxtFile(EXPORT_FILE, jsonStr);
                Toast.makeText(ctx, "已导出 " + kl.list.length + " 个密钥到 " + EXPORT_FILE, Toast.LENGTH_LONG).show();
            } catch (e) {
                Toast.makeText(ctx, "导出失败: " + e.toString(), Toast.LENGTH_SHORT).show();
            }
        }

        // 导入密钥：列出本地 密钥导出_*.json 文件供选择，合并（重名跳过）
        function importKeysDialog() {
            // 用 ttsrv.fileExist 探测最近 365 天内可能存在的导出文件（不依赖未暴露的 getFile）
            var exportFiles = [];
            var sdf = new java.text.SimpleDateFormat("yyMMdd");
            ...
            for (var back = 0; back < 365; back++) {
                ...
                var dateStr = sdf.format(cal.getTime());
                var fn = "密钥导出_" + dateStr + ".json";
                try {
                    if (ttsrv.fileExist(fn)) exportFiles.push(fn);
                } catch (e) {}
            }
            // 按文件名降序（日期新的在前）
            exportFiles.sort(function(a, b) { return b.localeCompare(a); });

            if (exportFiles.length === 0) {
                Toast.makeText(ctx, "未找到密钥导出文件，请先导出", Toast.LENGTH_SHORT).show();
                return;
            }

            // 读取并导入指定文件（解析 → 预览 → 确认 → 合并）
            function doImport(fileName) {
                var fileContent = null;
                try {
                    fileContent = ttsrv.readTxtFile(fileName);
                } catch (e) {
                    Toast.makeText(ctx, "读取文件失败", Toast.LENGTH_SHORT).show();
                    return;
                }
                if (!fileContent || !fileContent.trim()) {
                    Toast.makeText(ctx, fileName + " 内容为空", Toast.LENGTH_SHORT).show();
                    return;
                }
                var arr;
                try {
                    ...
                    if (!Array.isArray(arr)) {
                        Toast.makeText(ctx, "文件格式错误：不是数组", Toast.LENGTH_SHORT).show();
                        return;
                    }
                } catch (e) {
                    Toast.makeText(ctx, "文件内容解析失败", Toast.LENGTH_SHORT).show();
                    return;
                }
                var kl = getKeyMapAndList();
                var keyMap = kl.map;
                var keyOrder = kl.list.slice();
                var wouldAdd = 0, wouldSkip = 0;
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i] && arr[i].length >= 2) {
                        var name = (arr[i][0] || "").trim();
                        var item = arr[i][1];
                        if (name && item && item.value) {
                            if (keyMap.hasOwnProperty(name)) wouldSkip++;
                            else wouldAdd++;
                        }
                    }
                }
                if (wouldAdd === 0) {
                    Toast.makeText(ctx, "无新密钥可导入（" + wouldSkip + " 条已存在）", Toast.LENGTH_LONG).show();
                    return;
                }
                new android.app.AlertDialog.Builder(ctx)
                    .setTitle("导入确认")
                    .setMessage("从 " + fileName + " 读取 " + arr.length + " 条\n将新增 " + wouldAdd + " 个，跳过 " + wouldSkip + " 个重名")
                    .setPositiveButton("导入", new android.content.DialogInterface.OnClickListener({
                        onClick: function(d) {
                                ...
                                if (arr[i] && arr[i].length >= 2) {
                                    ...
                                    if (name && item && item.value) {
                                        if (keyMap.hasOwnProperty(name)) {
                                            ...
                                            keyMap[name] = { keyCode: item.keyCode || getNextKeyCode(keyMap), value: item.value };
                                            keyOrder.push(name);
                                            added++;
                                        }
                                    }
                            ...
                            Toast.makeText(ctx, "导入完成：新增 " + added + " 个，跳过 " + skipped + " 个重名", Toast.LENGTH_LONG).show();
                            ...
                            new android.os.Handler(android.os.Looper.getMainLooper()).post(new java.lang.Runnable({
                                run: function() {
                                    try {
                                        ...
                                        console.error("重新打开密钥管理弹窗失败：" + e.toString());
                                        Toast.makeText(ctx, "刷新失败，请重试", Toast.LENGTH_SHORT).show();
                                    }
                                }
                            }));
                        }
            ...
            if (exportFiles.length === 1) {
                doImport(exportFiles[0]);
            } else {
                // 转成 Java String[] 以兼容 setItems
                var fileArray = java.lang.reflect.Array.newInstance(java.lang.String.class, exportFiles.length);
                for (var i = 0; i < exportFiles.length; i++) {
                    fileArray[i] = exportFiles[i];
                }
                var builder = new android.app.AlertDialog.Builder(ctx);
                builder.setTitle("选择要导入的文件");
                builder.setItems(fileArray, new android.content.DialogInterface.OnClickListener({
                    onClick: function(d, which) {
                        doImport(exportFiles[which]);
                    }
                }));
                builder.setNegativeButton("取消", null);
                builder.show();
        ...
        function showModelSelectDialog(baseUrl, apiKey, models, targetIfc) {
            ...
            function rebuildList() {
                ...
                for (var g = 0; g < groupOrder.length; g++) {
                    ...
                    var titleParams = new android.widget.LinearLayout.LayoutParams(
                        0, android.widget.LinearLayout.LayoutParams.WRAP_CONTENT, 1
                    );
            ...
            builder.setView(root);
            builder.setPositiveButton("生成密钥", new android.content.DialogInterface.OnClickListener({
                onClick: function(d) {
                    var selected = [];
                    for (var m in checked) {
                        if (checked.hasOwnProperty(m) && checked[m] && !existingNames[m]) {
                            selected.push(m);
                        }
                    }
                    ...
                    for (var i = 0; i < selected.length; i++) {
                        var mName = selected[i];
                        if (keyMap.hasOwnProperty(mName)) {
                            skipped++;
                        } else {
                            keyMap[mName] = {
                                keyCode: getNextKeyCode(keyMap),
                                value: baseUrl + "@@" + mName + "@@" + apiKey
                            };
                            keyOrder.push(mName);
                            added++;
                        }
                    ...
                    Toast.makeText(ctx, "已生成 " + added + " 个密钥" + (skipped > 0 ? "，跳过 " + skipped + " 个重名" : ""), Toast.LENGTH_LONG).show();
                    d.dismiss();
                    if (keyManageDialog) { keyManageDialog.dismiss(); keyManageDialog = null; }
                    // 延迟到下一消息循环再打开，避免连续 dismiss 后同步 show 触发 WindowManager 异常
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(new java.lang.Runnable({
                        run: function() {
                            try {
                                showKeyManageDialog();
  ...
  // ↑ 修复结束
        ...
        var keyManageBtn = createRoundedButton("🔑  密钥管理", RMTHEME.cur.main);
        ...
        var backupRestoreButton = createRoundedButton("💾  备份恢复", RMTHEME.cur.main);
        ...
        ttsrv.setMargins(bookSectionLayout, 8, 12, 8, 0);
        ...
        bookBorder.setCornerRadius(dipToPx(12));
        ...
        function showBookSwitchDialog() {
            ...
            for (var si = 0; si < displayList.length; si++) {
                (function(sBookName, sIdx) {
                    ...
                    if (isCurrent) {
                        sbg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.tint));
                        sbg.setStroke(dipToPx(1.5), android.graphics.Color.parseColor(RMTHEME.cur.main));
                    } else {
            ...
            addBookRow.setTextColor(android.graphics.Color.parseColor(RMTHEME.cur.main));
            ...
            try {
                ...
                for (var bi = 0; bi < bookList.length; bi++) {
                    (function(position) {
                        ...
                        if (isCurrent) {
                            cardBg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.tint));
                        ...
                        if (isCurrent) {
                            ...
                            tagBg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.main));
        ...
        function showBackupRestoreDialog() {
            var optionConfigs = [
                { text: "导出当前书籍到剪贴板", color: RMTHEME.cur.main, icon: "" },
        ...
        function backupToClipboard() {
            try {
                ...
                Toast.makeText(ctx, "已复制到剪贴板：" + currentBookName + "（含角色数据）", Toast.LENGTH_SHORT).show();
            } catch (e) {
                console.error("书籍导出到剪贴板失败: " + e.toString());
                Toast.makeText(ctx, "导出失败: " + e.toString(), Toast.LENGTH_SHORT).show();
            }
        }
        ...
        ttsrv.setMargins(searchSectionLayout, 8, 12, 8, 0);
        ...
        var searchBg = new android.graphics.drawable.GradientDrawable();
        searchBg.setShape(android.graphics.drawable.GradientDrawable.RECTANGLE);
        searchBg.setCornerRadius(dipToPx(12));
        ...
        ttsrv.setMargins(mergeListView, 8, 2, 8, 0);
        ...
        addRowLp.setMargins(dipToPx(8), dipToPx(2), dipToPx(8), dipToPx(2));
        ...
        function createCircleDrawable(checked) {
            ...
            if (checked) {
                d.setColor(android.graphics.Color.parseColor(RMTHEME.cur.main));
        ...
        function createListRow(displayText, position, record) {
            ...
            if (record && record.voice) {
                ...
                var _mgTag = record.voice;
                    ...
                    voiceBg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.tint));
                    ...
                    charPvBtn.setTextColor(android.graphics.Color.parseColor(RMTHEME.cur.accent2));
        ...
        function showFirstDialog(position) {
            ...
            if (markedIndices.length >= 2) {
                optionConfigs.push({ text: "合并+跟随角色", color: RMTHEME.cur.main, icon: "", action: "merge_follow" });
                            ...
                            (function(fname, fidx) {
                                ...
                                if (fCharRecord && fCharRecord.voice) {
                                    ...
                                    if (fVoiceTag) {
                                        ...
                                        fVoiceBg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.tint));
        ...
        function showVoiceManageDialog(voiceTag, onChange) {
            ...
            try {
                ...
                function applyMarkBtnStyle(btn, mark, isSelected) {
                    ...
                    if (mark === "__delete__") {
                        bg.setColor(android.graphics.Color.parseColor("#FFEBEE"));
                        bg.setStroke(dipToPx(1), android.graphics.Color.parseColor("#FFCDD2"));
                    } else {
                ...
                for (var mi = 0; mi < markOptions.length; mi++) {
                    (function(mcfg) {
                        ...
                        if (mcfg.mark === "__delete__") {
                            markBtn.setTextColor(android.graphics.Color.parseColor("#E53935"));
                        }
                    ...
                    run: function () {
                        try {
                            ...
                            if (started) {
                                ...
                                // 不轮询、不跨线程写JS变量；UI更新经 _pvHandler 发主线程。
                                // 先等真正出声(→■)，再等播完(→▶)，任一步失败都复位。
                                var heard = false;
                                try { heard = ttsrv.awaitTtsPreviewAudible(30000); } catch (eW1) { heard = false; }
                                if (heard) {
                                    _pvHandler.post(new java.lang.Runnable({ run: function () { _pvPlay(tag, tag, btn); } }));
                                    try { ttsrv.awaitTtsPreviewDone(120000); } catch (eW2) {}
                                }
                                // 未出声即结束=合成失败(app已Toast)；极旧app无等待方法同样走复位
        ...
        function showFilteredVoiceDialog(voiceList, callback, _filterKeyword) {
            ...
            for (var vi = 0; vi < voiceOptions.length; vi++) {
                (function(vopt, vidx) {
                    ...
                    vtext.setText(truncateVoiceName(vopt.name));
                    ...
                    pvBtn.setTextColor(android.graphics.Color.parseColor(RMTHEME.cur.main));
        ...
        function showKeywordSelectionDialog(callback) {
            ...
            addBtn.setTextColor(android.graphics.Color.parseColor(RMTHEME.cur.main));
            ...
            addBtnBg.setColor(android.graphics.Color.parseColor(RMTHEME.cur.tint));
