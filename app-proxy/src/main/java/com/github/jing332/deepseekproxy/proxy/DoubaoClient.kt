...
object DoubaoClient {
    ...
    // 固定设备指纹（与 doubao2api.js 默认一致）
    private const val DEVICE_ID = "714003710229497"
    private const val WEB_ID = "7604137868021548590"
    private const val FP = "verify_mlcfw5f7_TPq0YmFD_NrsC_4RuQ_BJPg_M5W7i58I7wV0"
    ...
    // ── Security Params ─────────────────────────────────────
    private fun securityParams(): Map<String, String> {
        val params = linkedMapOf(
            "aid" to "582478",
            "real_aid" to "582478",
            "device_id" to DEVICE_ID,
            "tea_uuid" to DEVICE_ID,
            "web_id" to WEB_ID,
    ...
    // ── 聊天 payload（桌面客户端协议）─────────────────────────
    private fun buildCompletionPayload(text: String, needDeepThink: Int): String {
    ...
    fun chatCompletion(text: String, cookies: Map<String, String>, needDeepThink: Int = 0): Response {
        ...
        val req = Request.Builder().url(url)
            .post(buildCompletionPayload(text, needDeepThink).toRequestBody("application/json".toMediaType()))
